...
function TableHead({ className, ...props }: React.ComponentProps<'th'>) {
  return (
    <th
      ...
      className={cn(
        'text-muted-foreground h-9 px-2.5 text-left align-middle text-xs font-medium whitespace-nowrap [&:has([role=checkbox])]:pr-0',
        className,
      )}
      {...props}
    />
...
function TableCell({ className, ...props }: React.ComponentProps<'td'>) {
  return <td data-slot="table-cell" className={cn('px-2.5 py-2 align-middle [&:has([role=checkbox])]:pr-0', className)} {...props} />;
}
